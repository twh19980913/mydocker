...
// Delete the AUFS filesystem while containner exit
...
func DeleteMountPointWithVolume(volumeURLs []string, containerName string) error {
	...
	if _, err := exec.Command("umount", containerUrl).CombinedOutput(); err != nil {
		log.Errorf("Unmount volume %s failed.%v", containerUrl, err)
		return err
	}
	// cmd.Stdout = os.Stdout
	// cmd.Stderr = os.Stderr
	...
	if _, err := exec.Command("umount", mntURL).CombinedOutput(); err != nil {
		log.Errorf("Unmount mountpoint %s failed.%v", mntURL, err)
		return err
	}
	// cmd.Stdout = os.Stdout
	// cmd.Stderr = os.Stderr
...
func DeleteWriteLayer(containerName string) {
	...
	if err := os.RemoveAll(writeURL); err != nil {
		log.Errorf("Remove writeLayer %s error %v", writeURL, err)
	}
}
